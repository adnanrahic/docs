...
export const meta = {
  ...
  lastEdited: '2020-07-10T15:00:57.000Z',
}

The [<ProductName /> for GitLab integration](/docs/v2/integrations/vercel-for-gitlab) provides you with automatic deployments for each push to a project, automatic preview URLs for pushes to the [Production Branch](/docs/v2/git-integrations#production-branch), and instant rollbacks for reverts.
...
<Caption style={{ marginTop: '-40px' }}>
  A deployed commit using <ProductName /> for GitLab.
</Caption>
...
[Merge requests](https://docs.gitlab.com/ee/user/project/merge_requests/) are a way to submit changes to a project without pushing directly to the [Production Branch](/docs/v2/git-integrations#production-branch). This comes in handy when you're working on a feature that is either in progress or needs a review before being launched into production.
...
Pushing or merging to the [Production Branch](/docs/v2/git-integrations#production-branch) will result in a deployment, like any other branch or push. However, if you have a production domain set in the **Domains** section of your Project Settings, these deployments will be automatically available at your configured destination.
...
When you push this change to the [Production Branch](/docs/v2/git-integrations#production-branch), this and all subsequent commits to the branch will have the following comment, letting you know that your deployment is available in Production:
...
With every push, an automatic deployment. With every merge request, a unique preview URL. And with every update to the [Production Branch](/docs/v2/git-integrations#production-branch), your changes are updated for the Production Domain.
