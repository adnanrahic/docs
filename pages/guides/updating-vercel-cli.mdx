import Guide from '~/components/layout/guide'
import Snippet from '~/components/snippet'
import { InlineCode } from '~/components/text/code'
import Caption from '~/components/text/caption'
import ProductShortName from '~/components/name/product-short-name'
import ProductName from '~/components/name/product-name'
import { PRODUCT_NAME, PRODUCT_SHORT_NAME } from '~/lib/constants'

export const meta = {
  title: `How to Update ${PRODUCT_SHORT_NAME} CLI`,
  description: `Update ${PRODUCT_SHORT_NAME} CLI using a package manager.`,
  published: '2019-03-20T03:00:00.860Z',
  authors: ['mcsdev', 'timothy'],
  url: '/guides/updating-vercel-cli',
  editUrl: 'pages/guides/updating-vercel-cli.mdx',
  lastEdited: '2020-07-17T05:49:23.000Z',
}

When there is a new release of the [<ProductShortName /> CLI](/download) available, you will see a message appear in your terminal output when running <ProductName />, letting you know that there is a newer version.

This guide will take you through your options for updating <ProductShortName /> CLI using your preferred package manager.

## Option 1: Updating via npm

If you have installed our command-line interface via [npm](http://npmjs.org/), the easiest way to update it is by running the installation command yet again:

<Snippet width="100%" dark text="npm i -g vercel" />
<Caption>
  Updating <ProductShortName /> CLI using the <InlineCode>npm</InlineCode>{' '}
  command.
</Caption>

In the case that permission errors are preventing you from installing, please read npm's [official guide](https://docs.npmjs.com/resolving-eacces-permissions-errors-when-installing-packages-globally) about this problem.

## Option 2: Updating via Yarn

If you have installed our command-line interface via [Yarn](https://yarnpkg.com), the easiest way to update it is by running the installation command yet again:

<Snippet width="100%" dark text="yarn global add vercel" />
<Caption>
  Updating <ProductShortName /> CLI using the <InlineCode>yarn</InlineCode>{' '}
  command.
</Caption>

In the case that permission errors are preventing you from installing, please read npm's [official guide](https://docs.npmjs.com/resolving-eacces-permissions-errors-when-installing-packages-globally) about this problem (Yarn depends on the same configuration).

export default ({ children }) => <Guide meta={meta}>{children}</Guide>

export const config = {
  amp: 'hybrid',
}
