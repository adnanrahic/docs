...
<Caption>Adding an A record to a custom domain with a record value of <InlineCode>127.0.0.1</InlineCode> using the Vercel CLI.</Caption>

Another example would be adding an MX record for the root domain:

<Snippet dark text="vercel dns add example.com '@' mail.vercel.rocks 10" />
<Caption>Adding an MX record to a custom domain with a record value of <InlineCode>mail.vercel.rocks</InlineCode> using the Vercel CLI.</Caption>
...
<Caption>Listing the DNS records set for all domains under a Vercel account using Vercel CLI.</Caption>
...
<Caption><strong>Incorrectly adding</strong> an A record to a custom domain with a record value of <InlineCode>127.0.0.1</InlineCode> using the Vercel CLI.</Caption>

To solve this problem, [remove the record](#removing-dns-records) and it again **without** the domain as the "name" argument:

<Snippet dark text="vercel dns add example.com api A 127.0.0.1" />
<Caption><strong>Correctly adding</strong> an A record to a custom domain with a record value of <InlineCode>127.0.0.1</InlineCode> using the Vercel CLI.</Caption>
