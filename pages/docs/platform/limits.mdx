...
export const meta = {
  ...
  editUrl: 'pages/docs/platform/limits.mdx',
  lastEdited: '2020-08-26T14:28:14.000Z',
}
...
| Serverless Functions Created per Month       | 160   | 640  | Custom     |
...
The maximum size for a [Serverless Function](/docs/v2/serverless-functions/introduction) is **50 MB** and the maximum unzipped size is **250 MB** including layers which are automatically used depending on [Runtimes](/docs/runtimes#official-runtimes).
...
You are able to build `32` [Deployments](/docs/v2/platform/deployments) every `3600` seconds (1 hour). Should you hit the rate limit, you will need to wait another hour before you can build a deployment again.
